...
$rootScope.auth.$onAuthStateChanged(function(firebaseUser){if(firebaseUser=JSON.parse(JSON.stringify(firebaseUser))){var role,role;if(!firebaseUser||null==firebaseUser||!firebaseUser.email||void 0===$rootScope.config.admins[firebaseUser.email])
// There is no non-admin acceess
return role={role:"student"},$state.go("login",{msg:"Sorry, you do not have access."});role=$rootScope.config.admins[firebaseUser.email],firebaseUser.role=role,firebaseUser.time=new Date,firebaseUser.providerData=void 0}else role=null;if($rootScope.firebaseUser!=firebaseUser&&($rootScope.firebaseUser=firebaseUser,$window.localStorage.setItem("firebaseUser",JSON.stringify(firebaseUser))),firebaseUser&&"login"===$state.current.name&&"admin"===role.role){var params=void 0!=role.teacher?{query:role.teacher}:{};$state.go("students",params)}});
